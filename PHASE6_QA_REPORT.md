# Phase 6 QA Report — RawVault Real Provider Upload Execution
...
Route (app)
...
├ ƒ /api/uploads/dispatch
└ ƒ /api/uploads/execute
```
...
| `fileName` | string | Yes | Falls back to `file.name` if omitted |
| `sizeBytes` | string (numeric) | Yes | Must match actual file size |
...
- Max file size: **50 MB**
- `sizeBytes` must equal actual file byte length (SIZE_MISMATCH otherwise)
- `fileName` must be non-empty after trim
...
```bash
# 1. Obtain session cookie (sign in via UI or seeded-signin)
...
| Empty `fileName` | 400 | VALIDATION_ERROR |
...
| 2 | Phase 4 migration not applied | Run `supabase db push` or apply `20260313000400_phase4_provider_oauth_token_lifecycle.sql` |
...
- Build: pass  
- Route present: yes  
- Contract and error handling: aligned with PLAN/PHASE6_PLAN
