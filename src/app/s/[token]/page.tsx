  ...
  if (error) {
    return (
      ...
      >
        ...
        <AppButton variant="outline" size="sm" onClick={loadFolder}>
          {t("s.retry")}
        </AppButton>
      </div>
    );
        ...
        {items.map((item) => (
          <li key={item.id} role="listitem">
            {item.isFolder ? (
              ...
              <AppCard glass className="flex items-center gap-3 p-4">
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-4">
        ...
        >
          ...
          <AppButton variant="outline" size="sm" onClick={() => void fetchResolve()}>
            {t("s.retry")}
          </AppButton>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background pb-[env(safe-area-inset-bottom)]">
      <header className="rv-share-header">
        <PageShell surface="marketing" className="px-0 py-0">
          <PageHeader
            surface="marketing"
            title={<h1 className={cn("rv-share-title px-2 text-base sm:text-lg")}>{t("s.sharedWithYou")}</h1>}
            className="gap-0"
          />
        </PageShell>
      </header>
      <main className="flex-1 animate-enter" role="main">
        <PageShell surface="marketing" className="px-3 py-6 sm:px-4 sm:py-8">
